...
export function InventoryCard({ product, onSell, onEdit, onDelete }: InventoryCardProps) {
  ...
  const opacity = useTransform(x, [-150, -100, 0, 100, 150], [0.8, 1, 1, 1, 0.8]);
  const backgroundColor = useTransform(
    x,
    [-150, -100, 0, 100, 150],
    ["rgba(34, 197, 94, 0.2)", "rgba(34, 197, 94, 0.1)", "rgba(255, 255, 255, 0)", "rgba(59, 130, 246, 0.1)", "rgba(59, 130, 246, 0.2)"]
  );
  ...
  const bind = useDrag(
    ({ movement: [mx], velocity: [vx], last }) => {
      if (last) {
        // Swipe completed
        if (mx < -100 && Math.abs(vx) > 0.5) {
          // Swipe left - Sell
          onSell(product);
          x.set(0);
          setSwipeDirection(null);
        } else if (mx > 100 && Math.abs(vx) > 0.5) {
          // Swipe right - Edit
          onEdit(product);
          x.set(0);
          setSwipeDirection(null);
  ...
  return (
      <div
        ref={cardRef}
        style={{ x: x.get(), opacity: opacity.get(), backgroundColor: backgroundColor.get() }}
      ...
      <motion.div
        layout
        className="relative bg-card border border-border rounded-xl overflow-hidden shadow-sm z-10"
